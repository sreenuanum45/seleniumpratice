...
public class CarouselSliderUtility {
	public boolean AreSliderMovingAutomaticallyInSlider(WebElement slider) throws Throwable {
		...
		if(z!=null) {
			...
			else if (z.equals("center")) {
				System.out.println("center");
				return true;
			}

			else if (z.equals("justify")) {
				System.out.println("justify");
				return true;
			}

			else if (z.equals("initial")) {
	...
	public  String getSliderMoveDuration(WebElement slider) throws Exception {
		String transitionDuration = slider.getCssValue("transition-duration");
		String transitionTimingFunction = slider.getCssValue("transition-timing-function");
		String transitionDelay = slider.getCssValue("transition-delay");
		String x=slider.getCssValue("transition");
		String []y=x.split(" ");
		y[1]=y[1].trim();
		if(y[1].contains("ms"))
	...
	public void pauseSlider(WebElement slider) {


		slider.sendKeys(Keys.SPACE);
		// This works if the slider can be paused by space key
	}

	public void resumeSlider(WebElement slider) {
		slider.sendKeys(Keys.SPACE); // This works if the slider can be resumed by space key
	}

	public boolean isSliderPaused(WebElement slider) throws Throwable {
		// Pause slider to check if it was already paused
		pauseSlider(slider);
		Thread.sleep(1000);
		String transformBefore = slider.getCssValue("transform");

		// Resume slider and check if it moves
		resumeSlider(slider);
		Thread.sleep(1000);
		String transformAfter = slider.getCssValue("transform");

		return transformBefore.equals(transformAfter);
	}
	public void moveToSlide(WebElement slider, int slideIndex) {
		List<WebElement> slides = slider.findElements(By.xpath("child::li"));
		if (slideIndex >= 0 && slideIndex < slides.size()) {
			slides.get(slideIndex).click();
		}
		else {
			System.out.println("Invalid slide index");
		}
	}
